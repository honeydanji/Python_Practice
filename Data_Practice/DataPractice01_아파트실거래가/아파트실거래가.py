...
apt = opencsv('C:\myProject\Python\Python_Practice\Data_Practice\데이터전처리_Practice/apt_202306.csv')
...
for i in apt:
    if float(i[5].replace(',', '')) >= 120 and int(i[8].replace(',', '')) <= 30000 and re.match('부산', i[0]):
        # replace를 사용해서 문자열을 정리해줘야한다. 그래야 int()를 사용할 수 있다.
        new_list.append([i[0], i[5], i[8]]) # list에 추출한 값 저장하기.
    
writecsv('over120_lower30000.csv', new_list) # 리스트를 csv 파일로 저장하기.